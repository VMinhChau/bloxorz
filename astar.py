...
class Astar:
     ...
     def add_move_astar(self, expanded_nodes, block, heuristic_costs):
          if process_state(block):
               g_cost = self.get_cost_visited((block.prev.x,block.prev.y,block.prev.status)) + 1
               if ((block.x,block.y,block.status) not in self.cost_visited) or g_cost < self.get_cost_visited((block.x,block.y,block.status)):
     ...
     def solve_by_astar(self, block):
          ...
          self.set_cost_visited((block.x,block.y,block.status), 0)
